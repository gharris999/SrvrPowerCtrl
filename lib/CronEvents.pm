# Adapted from Schedule::Cron::Events, Copyright 2002 P Kent
# http://search.cpan.org/~kohts/Schedule-Cron-Events-1.9/lib/Schedule/Cron/Events.pm
# http://cpansearch.perl.org/src/KOHTS/Schedule-Cron-Events-1.9/lib/Schedule/Cron/Events.pm

#package CronEvents;
package Plugins::SrvrPowerCtrl::lib::CronEvents;
...
sub cronLineToRanges {
	...
	my $s = new crontab( $crondate[0], [0..59] );
	#my $s = crontab->new( $crondate[0], [0..59] );

	$ranges{'minutes'} = [ $s->list() ];

	# $s = new Set::Crontab( $crondate[1], [0..23] );
	$s = new crontab( $crondate[1], [0..23] );
	$ranges{'hours'} = [ $s->list() ];

	# $s = new Set::Crontab( $crondate[2], [0..31] );
	$s = new crontab( $crondate[2], [0..31] );
