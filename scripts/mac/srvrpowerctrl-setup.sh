...
#####################################################################################################
#
# Load the helper functions.. (OS X has no readlink -f)
#
...
cd_to_script_dir(){
	...
	if [ ! "$SOURCEDIR" = "$CWD" ]; then
		echo "Error: could not change to directory ${SOURCEDIR}...exiting!"
		echo "${SOURCEDIR} != ${CWD}"
		exit 1
	fi
	return 0
...
fix_plugin_permissions(){
    ...
    if  ( _index "$PARENT_DIR" 'InstalledPlugins' ); then
		echo "No need to fix permissions.."
		return 1
	fi
	...
	USER=$(stat -Ll "$PARENT_DIR" | awk '{print $3}')
...
install_scripts(){
	...
	for SCRIPTNAME in spc-*
	do
		if [ -e "${TARGET_DIR}/${SCRIPTNAME}" ]; then
			mv -f "${TARGET_DIR}/${SCRIPTNAME}" "${TARGET_DIR}/${SCRIPTNAME}.bak"
		fi
		...
		# Fixup permissions
		chown root:wheel "${TARGET_DIR}/${SCRIPTNAME}"
...
echo 'Setting up SrvrPowerCtrl plugin helper scripts for OS X..'
