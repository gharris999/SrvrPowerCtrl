...
spc_get_slim_service_name(){
	...
	if [ -z "$SLIMSERVICENAME" ]; then
		SLIMSERVICENAME=`pgrep -fln 'perl.*/squeeze|perl.*/slim|perl.*/logitech' | sed -e 's#^.*/usr/share/\([^/]*\).*$#\1#' -e 's#^.*/usr/sbin/\([^ ]*\).*$#\1#'`
	fi
...
spc_stop_slim_service(){
	...
	SLIMSERVICEPROC=$(pgrep -fln 'perl.*/squeeze|perl.*/slim|perl.*/logitech' | sed -e 's#^.*/usr/share/\([^/]*\).*$#\1#' -e 's#^.*/usr/sbin/\([^ ]*\).*$#\1#')
...
spc_get_slim_localrepo(){
	...
	ISGIT=$(pgrep -fn "perl.*/usr/share/${SLIMSERVICENAME}/server/slim" | wc -l)
...
spc_get_slim_cachedir(){
	SLIMCACHEDIR=`pgrep -fln 'perl.*/squeeze|perl.*/slim|perl.*/logitech' | sed -n -e 's#^.*--cachedir[[:space:]]*\([^[:space:]]*\).*$#\1#p'`
	# Punt
...
spc_get_slim_log_dir(){
	  SLIMLOGDIR=`pgrep -fln 'perl.*/squeeze|perl.*/slim|perl.*/logitech' | sed -n -e 's#^.*--logdir[=]*[ ]*\(/[^ ]*\).*$#\1#p'`
